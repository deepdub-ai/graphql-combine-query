...
export interface NewCombinedQueryBuilder {
  ...
  add: <TData = any, TVariables = OperationVariables>(
    document: DocumentNode,
    variables?: TVariables
  ) => CombinedQueryBuilder<TData, TVariables>;
  addN: <TVariables = OperationVariables>(
    document: DocumentNode,
    variables: TVariables[],
...
> {
  document: DocumentNode;
  variables?: TVariables;
  add: <TDataAdd = any, TVariablesAdd = OperationVariables>(
    document: DocumentNode,
...
class CombinedQueryBuilderImpl<TData = any, TVariables = OperationVariables>
  implements CombinedQueryBuilder<TData, TVariables>
{
  document: DocumentNode;
  variables?: TVariables;
  ...
  add<TDataAdd = any, TVariablesAdd = OperationVariables>(
    ...
    const opDefs = this.document.definitions
    ...
    if (!opDefs.length) {
      throw new CombinedQueryError(
        "Expected at least one OperationDefinition, but found none."
      );
    }
        ...
        .forEach((sel) => {
          otherOpDefs.forEach((_def) =>
            _def.selectionSet.selections
              ...
              .forEach((_sel) => {
                if (
                  (sel.alias?.value || sel.name.value) ===
                  (_sel.alias?.value || _sel.name.value)
                ) {
                  throw new CombinedQueryError(
      ...
      def.variableDefinitions?.forEach((variable) => {
        otherOpDefs.forEach((_def) =>
          _def.variableDefinitions?.forEach((_variable) => {
            if (
              variable.variable.name.value === _variable.variable.name.value
            ) {
              throw new CombinedQueryError(
    ...
    let definitions: DefinitionNode[] = [
      {
        ...
        variableDefinitions: opDefs.flatMap(
          (def) => def.variableDefinitions || []
        ),
      },
    ];
    const encounteredFragmentList = new Set<string>();
    const combinedDocumentDefinitions = this.document.definitions.concat(
    ...
    return new CombinedQueryBuilderImpl<
      TData & TDataAdd,
      TVariables & TVariablesAdd
    >(this.operationName, newDoc, newVars);
  }
...
export default function combinedQuery(
  operationName: string
): NewCombinedQueryBuilder {
  return {
    operationName,
    add<TData = any, TVariables extends OperationVariables = {}>(
      document: DocumentNode,
      variables?: TVariables
    ) {
      return new CombinedQueryBuilderImpl<TData, TVariables>(
        this.operationName,
        document,
        variables
      );
